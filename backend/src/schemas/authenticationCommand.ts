      ...
      if (checkToken) {
        if (user.verificationCode.expiresAt < Date.now()) {
          ...
          throw new APIError({
            status: 401,
            message:
              "Opps... your token has been expired. Please provide new token sent to your email.",
          });
        } else {
        ...
        throw new APIError({
          status: 401,
          message:
            "Opps... your token is not correct. Please provide correct one.",
        });
      }
    } else {
      throw new APIError({
      ...
      if (checkToken) {
        if (user.verificationCode.expiresAt < Date.now()) {
          ...
          throw new APIError({
            status: 401,
            message:
              "Opps.. your token has been expired. A new token has been sent to your email address.",
          });
        } else {
          const salt = await bcrypt.genSalt(10);
          const hashedPassword = await bcrypt.hash(this.password, salt);
        ...
        throw new APIError({
          status: 401,
          message:
            "Opps... your token is not correct. Please provide proper otp.",
        });
      }
    } else {
      throw new APIError({
...
export class CheckEmailAndSendOtpCommand implements ICommand {
  email: string;
  ...
  async execute() {
      ...
      const result = await updateEmailVerificationCode(this.email, verificationCode);
      await sendEmailVerificationCode(this.email, token);
      if (!result) {
        throw new APIError({
          status: 400,
          message: "Opps... email not found.",
